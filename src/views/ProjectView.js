...
const ProjectView = () => {
  ...
  if(project === 1) {
    ...
    content = (
      <>
        <h3>The problem</h3>
        <p>The lack of citizen-centric project tracking solutions for government-run public works was the gap the CDPT system attempts to tackle. Currently, there are very few (and scarcely maintained) sources of public projects data available to the general public. The present ones, predecessors to the CDPT, lean heavily towards technicalities that the layman - the common <i>mwananchi</i> - would most likely be non-conversant with.
        </p>
        <h3>The solution</h3>
        <p>To redefine the concept of public works project tracking, the solution had to adopt a focus for the person in the grassroots. What the CDPT system manages to do is offer itself up as a centralized platform for tracking development projects in all 47 counties, providing digestible insights for each. The interface makes for an easy-to-follow-and-understand experience, ensuring that all information that may be needed is only a few clicks away.</p>
        <h3>The approach</h3>
        <p>The system was developed for web due to the ubiquity and accessibility of the online medium. Web pages were maintained at a minimum for ease of use. The system architecture comprised of a frontend with public user and admin client UIs, a web server, a backend with a relational database and an intermediary RESTful API for data communications across the various levels.</p>
        <h3>The result</h3>
        <p>An online system capable of relaying public project information by facts and numbers to the general public.</p>
      </>
    )
    liveLink = (
    ...
    content = (
      <>
        <h3>The challenge</h3>
        <p>Provided with screenshots for the desktop and mobile experience, the task was to recreate the website with the specifications highlighted in the guides. Backend integration was not required but was included in this particular attempt, by way of a relational database queried using MySQL over a RESTful API. The frontend was hosted on GitHub using GitHub Pages and the backend deployed on Heroku.</p>
        <h3>The challenge's challenge</h3>
        <p>This project features on my portfolio by virtue of being a personal victory in my web development learning journey. First built using vanilla JavaScript in 2021 and (unsuccessfully) deployed, I came around to it months later with a better understanding of 'how things work', made things work, and now everything works - and not just on my machine.</p>
      </>
    )
    liveLink = (
